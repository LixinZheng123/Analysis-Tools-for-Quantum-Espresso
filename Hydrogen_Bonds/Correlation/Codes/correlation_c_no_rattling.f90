...
write(*,*) 'Output file  : ', trim(filename)//'.corr_c'
!
!
iiO_p=0
end subroutine init
...
open(unit=5, file=(trim(filename)//'.corr_c'), status='unknown')
open(unit=8, file=(trim(filename)//'.trans_no_r'), status='unknown')
...
!************************************
do 
  read(4,*,iostat=ierror) last_step,iiO0
  if (ierror .lt. 0) then
  ...
  !======
  if (iiO_p(2) .ne. iiO0 ) then
    last_step0=last_step
    write(8,*) last_step0, iiO0
    ncount=ncount+1
  endif
  iiO_p(2)=iiO_p(1)
  iiO_p(1)=iiO0
...
enddo
close(8)
open(unit=8, file=(trim(filename)//'.trans_no_r'), status='old')
!
!
last_step=last_step0
...
do dk=0, last_dk-1
  ...
  Cc=(real(uh(dk+1))/(last_k-last_dk))
  tau_c=tau_c+Cc
  !write(5,*) (dk*dstep), timestep*(dk*dstep), Ci, Cc
  write(5,*) (dk*dstep), timestep*(dk*dstep), Cc
enddo
