...
write(*,*) '------------------------------------------'
write(*,*) 'Input file       : ', trim(filename)//'.fss '
write(*,*) 'Output file          : ', trim(filename)//'.bond ',trim(filename)//'.hbcase'
...
select case(io)
  case(1)
    ...
    !open(unit=5, file=(trim(filename)//'.sigma'), status='old')
...
do
  ...
  !**********************************
  !Read .trans
  if (step_t .le. step+10) read(4,*,iostat=ierror) step_t,iiO_t
  if (ierror .lt. 0) then
    write(*,*) '  End of File Reached'
    write(*,fmt='(1X, "  Total number of Samples: ", I7)' )(ncount)
    exit
  ...
  !**********************************
  !Read .sigma
  !read(5,*) step_p,time,iiO,iiH_t,sigma,state
  !**********************************
  !Error message
  if (nsp(1) .eq. 0) then
    write(*,*) 'Error! nsp1=0.'
  ...
  !if (step_p .ne. step) then
  !  write(*,*) 'Error! Steps read not equal in .sigma and .fss file!'
  !  exit
  !endif
  !**********************************
  !Read .fss
  ...
  do i=1,nsp(1)
    read(3,*)  iO(i),rO(1:3,i),hnum(1:4,i)
    if (iO(i) .eq. iiO) then
      do j=1,3
        rIO(j)=rO(j,i)
      enddo
  ...
  write(7,*) step, time, hbcase
  if (hbcase .ne. 0) then
    if (HBIndex(1,1) .eq. HBIndex(2,1) .or. &
        HBIndex(2,1) .eq. HBIndex(3,1) .or. &
        HBIndex(3,1) .eq. HBIndex(4,1) .or. &
        HBIndex(1,1) .eq. HBIndex(3,1) .or. &
        HBIndex(2,1) .eq. HBIndex(4,1) .or. &
        HBIndex(1,1) .eq. HBIndex(4,1) )    &
        write(*,*) "Error! multiple accepting of 1 hydrogen!"
  endif
  write(7,*) HBIndex(1,1), HBIndex(2,1), HBIndex(3,1), HBIndex(4,1)
  write(7,*) HBIndex(1,2)
