...
program rdf 
   ...
   real(DP)             :: gofr,          &  !FINAL g(r) print-out variable (not an array, print-out values)  
                           ...
                           rO(3,64),rH(3,128),&!atomic positions (dimension,atomic-species)
   ...
   !
   namelist /input/ filename,filegofr, atom, nsp, binNum, stepstart, stepstop
   !
   !initialization
   call init
   !
...
write(*,*) '------------------------------'
write(*,*) '1st solvation file : ', trim(filename)//'.pos'
...
do 
  ...
  if (atom(1) .eq. 1 .and. atom(2) .eq. 1) then
    do i=1,nsp(1)-1
      do k=i+1,nsp(1)
        if (i .ne. k) then
          call get_distance(rO(1:3,i),rO(1:3,k),d)
          nbin = nint(d*res)
          if (nbin .gt. binNum) nbin = binNum
          temgrt(nbin) = temgrt(nbin) + 2
        endif
      enddo
    enddo
  endif
  ...
  if (atom(1) .eq. 1 .and. atom(2) .eq. 2) then
    do i=1,nsp(1)
      do k=1,nsp(2)
        call get_distance(rO(1:3,i),rH(1:3,k),d)
        nbin = nint(d*res)
        if (nbin .gt. binNum) nbin = binNum
...
do n=1,binNum-1,1
  ...
  if (atom(1) .eq. 1 .and. atom(2) .eq. 1) dens=DBLE((nsp(1)-1)/(omega))
  if (atom(1) .eq. 1 .and. atom(2) .eq. 2) dens=DBLE(nsp(2)/omega)
