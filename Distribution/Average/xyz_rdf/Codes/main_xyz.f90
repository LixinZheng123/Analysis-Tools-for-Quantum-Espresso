...
!
!Modified: Lixin Zheng
!
!Reads .xyz output file
!
!
!==================================================================================================
 PROGRAM new_rdf
   ...
   !
   namelist /input/ filename, binNum, ntsp, nsp, mic, &
                  stepstart, stepstop, atom1, atom2, norm, lattice
   !
   !
   !Start Time
   ...
   !********************************************************************************
      ...
      subroutine files(io)
         ...
         select case(io)
            case(1)
               open(unit=1, file=(trim(filename)//'.xyz'), status='old')
               !open(unit=2, file=(trim(fileCel)), status='old')
               open(unit=3, file=(trim(filename)//'.gOO'), status='unknown')
               !
               write(*,*)
      ...
      subroutine main
         ...
         Main_loop: do 
            ...
            if (readstep .lt. stepstart) cycle
            !-----Check for stop condition-----
            if (readstep .gt. stepstop) exit
            !
            do i=1,3
              do j=1,3
                aprim(i,j)=0
            ...
            do ns=1,ntsp,1
               do na=1,nsp(ns),1
                  read(1,*) dummyC, (tau(j,na,ns),j=1,3)   
               enddo
            enddo
            !
            !
