...
!
!
!!this is for remove of center-of-mass, to be added
!!Turns out that COM has already been removed in cp codes.
!  read(3,*)
!  do i=1, nsp1+nsp2
!    read(3,*) 
!  enddo
...
do dk=1,last_dk
  ...
  do k=k0,ncount-dk,5
    ...
    do j=1,3
      !
      dr=rIO(j,k+dk)-rIO(j,k)
      !dr=dr-nint(dr/celldm)*celldm
      !
      !
      !Adjust the parameter: box
      if (abs(dr-drp(j,k)) .gt. cutoff1 ) then
        if (dr-drp(j,k) .gt. 0) then
          box(j,k)=box(j,k)-1
        else 
          box(j,k)=box(j,k)+1
        endif
      ...
      !Situations that can go wrong
      else if (abs(dr-drp(j,k)) .gt. cutoff2 ) then
        !if (                                &
        ...
        !  !Error! dr-drp too big!
          write(4,*) step(k+dk),time(k+dk),abs(dr-drp(j,k)),j
        !endif
      endif
      !
      !
      ...
      dr_tot(j)=dr+box(j,k)*celldm
      drp(j,k)=dr
      !
    enddo
    !
    d2=(dr_tot(1))**2+(dr_tot(2))**2+(dr_tot(3))**2
    tot_sd=tot_sd+d2
    n=n+1
    !
    !
